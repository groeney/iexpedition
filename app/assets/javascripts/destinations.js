        ...
        function open_modal() {
            $('.region-select-js').on('click', function () {
                ...
                if (if_main_slider) {
                    ...
                    $(this).parents('.lb-album').find('[data-order=' + +order + ']').parent('.lb-overlay').css({
                        'width': whichSize()[0],
                        'top': -25,
                        'left': -whichSize()[1]
                    }).show().children('.lb-content').show();
                    disableButton()
                } else if ($('.lb-album').hasClass('not_offset')) {
                    order = $(this).parents('.wrap-li').find('.lb-overlay').css({
                        'width': whichSize()[0],
                        'top': -25,
                        'left': -whichSize()[1]
                    }).show().children('.lb-content').show().data('order');
                    disableButton()
                    return order
                } else {
        ...
        function next_button() {
            $('.next-lightbox').on('click', function () {
                if (order < sum) {
                    ...
                    if (if_main_slider) {
                        $('.flex-next').click()
                        $('[data-order=' + +order + ']').addClass('right').show().parent('.lb-overlay').css({
                            'width': whichSize()[0],
                            'top': -25,
                            'left': -whichSize()[1]
                        })
                    } else if ($('.lb-album').hasClass('not_offset')) {
                        $('[data-order=' + +order + ']').addClass('right').show().parent().css({
                            'width': whichSize()[0],
                            'top': -25,
                            'left': -whichSize()[1]
                        })
        ...
        function prev_button() {
            $('.prev-lightbox').on('click', function () {
                if ((order > 1) && (order <= sum)) {
                    ...
                    if (if_main_slider) {
                        $('.flex-prev').click()
                        $('[data-order=' + +order + ']').addClass('left').show().parent('.lb-overlay').css({
                            'width': whichSize()[0],
                            'top': -25,
                            'left': -whichSize()[1]
                        })
                    } else if ($('.lb-album').hasClass('not_offset')) {
                        $('[data-order=' + +order + ']').addClass('left').show().parent().css({
                            'width': whichSize()[0],
                            'top': -25,
                            'left': -whichSize()[1]
                        })
